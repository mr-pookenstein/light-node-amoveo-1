...
function bets_main() {
  ...
  window.upperlimit = "global";   // Declare a global variable
  window.lowerlimit = "global";   // Declare a global variable

  //eventually this will come from oracle question
  upperlimit = 0.05;
  lowerlimit = 0.00;
...
//                order.innerHTML = "in market ".concat(oid).concat(" you have an open order to trade this many tokens ").concat(s2c(amount)).concat(", you are trading at this price: ").concat(parseFloat(((bet[4][2])/100), 10)).concat(", you are betting on outcome: ").concat(outcome);
                  ...
                  console.log("bet2 "+bet[2]);


                  if (outcome == "true"){
                  order.innerHTML = ("ACTIVE ORDER - Remaining: ").concat(directionize(outcome)).concat(s2c(Number(amount) * Number(bet[4][2]) / (Number(10000) + Number(bet[4][2])))).concat(" VEO,  Order Price: ").concat(Math.round(parseFloat(((convertprices(bet[4][2]))/100), 10)* 10000) / 10000).concat("  ");

                }else{
                  order.innerHTML = ("ACTIVE ORDER - Remaining: ").concat(directionize(outcome)).concat(s2c(Number(amount) * Number(bet[4][2]) / (Number(10000) + Number(bet[4][2])))).concat(" VEO,  Order Price: ").concat(Math.round(parseFloat(((convertprices(10000 - bet[4][2]))/100), 10)* 10000) / 10000).concat("  ");

                }

console.log(amount);
console.log(s2c(amount));
                ...
                console.log("AMOUNT HERE "+s2c(amount));
                console.log("S2C AMOUNT HERE "+s2c(amount));
                console.log("BET42 AMOUNT HERE "+bet[4][2]);



                if (outcome == "true"){
                order.innerHTML = ("POSITION - Value: ").concat(directionize(outcome)).concat(s2c(Number(amount) * Number(bet[4][2]) / (Number(10000) + Number(bet[4][2])) )).concat(" VEO,  Entry Price: ").concat(Math.round(parseFloat(((convertprices(bet[4][2]))/100), 10)* 10000) / 10000);
              } else {
                order.innerHTML = ("POSITION - Value: ").concat(directionize(outcome)).concat(s2c(Number(amount) * Number(bet[4][2]) / (Number(10000) + Number(bet[4][2])))).concat(" VEO,  Entry Price: ").concat(Math.round(parseFloat(((convertprices(10000 - bet[4][2]))/100), 10)* 10000) / 10000);
              }
